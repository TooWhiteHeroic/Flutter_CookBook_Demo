...
import 'package:sqflite/sqflite.dart';//1. 添加依赖

void main() async {
  final database = openDatabase(//3. 打开数据库
    // 设置数据库的路径。注意：使用 `path` 包中的 `join` 方法是
    // 确保在多平台上路径都正确的最佳实践。
    join(await getDatabasesPath(), 'doggie_database.db'),
    // 当数据库第一次被创建的时候，创建一个数据表，用以存储狗狗们的数据。
    onCreate: (db, version) {
      return db.execute("CREATE TABLE dogs(id INTEGER PRIMARY KEY, name TEXT, age INTEGER)",//4. 创建 dogs 表
      );
    },
  ...
  Future<void> insertDog(Dog dog) async { //5. 插入一条狗狗的数据
  ...
  Future<List<Dog>> dogs() async {
    ...
    final List<Map<String, dynamic>> maps = await db.query('dogs');//6. 查询狗狗列表
  ...
  Future<void> deleteDog(int id) async {
    ...
    await db.delete(//8. 删除一条 Dog 的数据
      'dogs',
